...
void
animation_init_w_texture(Animation* anim, const Texture* texture, const AnimationTemplate* tmpl)
{

    int num_frames       = tmpl->column_count * tmpl->row_count;
    int idx              = 0;
    anim->frames         = SDL_malloc(sizeof(*anim->frames) * (size_t)num_frames);
    anim->frame_cnt      = num_frames;
...
void
animation_init_w_sprite(Animation* animation, const Sprite* sprite, const AnimationTemplate* tmpl)
{
...
int
animation_load_rwops(Animation* animation, SDL_RWops* stream)
{
    if (!animation || !stream)
    {
