...
void
update_local_transform_matrices_no_releationship(ecs_Registry* registry)
{
    struct ecs_View view;
    ...
    struct LocalTransformMatrixComp* local_to_world;
    ...
    while (ecs_view_next(&view, &ett, components))
    {
        ...
        glm_mat3_copy(matrix, local_to_world->value);
        ecs_add(registry, ett, WorldTransformMatrixChangedTag);
    }
}
...
{
    update_local_transform_matrices_no_releationship(registry);
